...
const batchPosts = [
  {
    ...
    promptGen1: 'high fashion editorial photography of Kendall Jenner in a black vintage leather trench coat on a rainy Paris street at night, Leica 50mm f1.4, photorealistic 8k',
    promptGen2: 'close up portrait of Kendall Jenner wearing retro sunglasses and black turtleneck in Paris street at dusk, cinematic street lights bokeh, 8k'
  },
  {
    ...
    promptGen1: 'Vogue summer lifestyle photography of a sun-kissed blonde woman in white linen on a luxury yacht near Amalfi Coast, Canon 85mm f1.2, photorealistic 8k',
    promptGen2: 'full body portrait of blonde model relaxing on a teak yacht deck with turquoise sea background, bright summer sunlight, ultra realistic 8k'
  }
];

async function downloadImage(promptText, imageName, seed) {
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(promptText)}?width=768&height=1024&nologo=true&model=flux&enhance=true&seed=${seed}`;
    ...
    https.get(url, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302) {
        https.get(res.headers.location, (res2) => {
          ...
          res2.pipe(file);
          file.on('finish', () => { file.close(); console.log(`Done: ${imageName}`); resolve(); });
        });
      } else {
        const file = fs.createWriteStream(destPath);
        res.pipe(file);
      ...
      console.error(`Download error ${imageName}:`, err.message);
      resolve();
    });
  });
}
...
async function run() {
  console.log('1. Downloading images for batch posts...');
  for (let i = 0; i < batchPosts.length; i++) {
    const b = batchPosts[i];
    await downloadImage(b.promptGen1, b.img1Name, 1000 + i);
    await downloadImage(b.promptGen2, b.img2Name, 2000 + i);
  }

  console.log('2. Updating db.json...');
  ...
  console.log('3. Sending batch posts to Telegram group...');
