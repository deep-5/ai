...
// 3 exact @pixelbyus prompts & image targets
const initial3Posts = [
  {
    id: 'pixelbyus-post-1',
...
async function downloadImage(item) {
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(item.promptText)}?width=768&height=1024&nologo=true&model=flux&enhance=true&seed=${item.seed}`;
  const destPath = path.join(uploadsDir, item.imageName);
    ...
    https.get(url, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302) {
        https.get(res.headers.location, (res2) => {
          ...
          res2.pipe(file);
          file.on('finish', () => { file.close(); console.log(`Done image: ${item.imageName}`); resolve(); });
        });
      } else {
        const file = fs.createWriteStream(destPath);
        res.pipe(file);
      ...
      console.error(`Download error ${item.imageName}:`, err.message);
      resolve();
    });
  });
}
...
function sendPhotoMultipart(p) {
  return new Promise((resolve) => {
    ...
    const imageFilePath = path.join(uploadsDir, p.imageName);
...
async function run() {
  console.log('1. Downloading 3 exact @pixelbyus images...');
  for (const item of initial3Posts) {
  ...
  // Remove any previous pixelbyus-post-* items and add new ones at the top
  const otherPrompts = dbData.prompts.filter(p => !p.id.startsWith('pixelbyus-post-'));
  dbData.prompts = [...formattedPosts, ...otherPrompts];
  fs.writeFileSync(dbPath, JSON.stringify(dbData, null, 2), 'utf8');
  console.log('db.json updated!');

  console.log('3. Sending 3 exact @pixelbyus posts to Telegram group...');
  for (const p of formattedPosts) {
    console.log(`Sending to Telegram: ${p.title}`);
  ...
  console.log('Initial 3 @pixelbyus posts setup complete!');
}
